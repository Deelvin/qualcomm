...
int RPCTimeEvaluator(TVMValue* args, int* type_codes, int num_args, TVMValue* ret_val,
  ...
  if (type_codes[0] != kTVMModuleHandle || type_codes[1] != kTVMStr ||
      ...
      type_codes[5] != kTVMArgInt || type_codes[6] != kTVMArgInt || type_codes[7] != kTVMArgInt || type_codes[8] != kTVMStr) {
  ...
  g_time_evaluator_state.run_delay = args[6].v_int64;
  g_time_evaluator_state.min_repeat_ms = args[7].v_int64;
...
tvm_crt_error_t RunTimeEvaluator(tvm_function_index_t function_index, TVMValue* args,
  ...
  // std::cout << "ICE RunTimeEvaluator ERROR" << std::endl;
