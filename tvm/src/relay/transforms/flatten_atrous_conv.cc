 ...
 public:
  Expr MutateSubgraph(const Expr& expr) {
    try {
      ...
      // Replace to "transform::InferTypeLocal(data).as<TensorTypeNode>()->shape;"
      // when InferTypeLocal will be added.      
      Array<PrimExpr> data_shape = data->type_as<TensorTypeNode>()->shape;
