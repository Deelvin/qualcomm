        ...
        if (fn->HasNonzeroAttr(attr::kPrimitive)) {
          ...
          if (primitive_supports_texture_) {
            if (call->checked_type().as<TensorTypeNode>()) {
              ...
              // TODO(amalyshe) this is strange behaviour since if we define it is not 5d
              // we do not rely on textures, but add it to the scope in any case
    ...
    if (auto attrs = call->attrs.as<Conv2DAttrs>()) {
      ...
      if (attrs->layout == "NCHW4c") {
        supports_texture_storage = true;
                ...
                call->op == Op::Get("nn.pad") ||
                call->op == Op::Get("divide") ||
                call->op == Op::Get("subtract") ||
                call->op == Op::Get("less") ||
                call->op == Op::Get("less_equal") ||
                call->op == Op::Get("greater") ||
                call->op == Op::Get("greater_equal") ||
                call->op == Op::Get("equal") ||
                call->op == Op::Get("not_equal")) {
      if (const auto* ttype = call->checked_type().as<TensorTypeNode>()) {
        if (ttype->shape.size() == 5) {
          supports_texture_storage = true;
        }
