...
def test_opencl_type_casting():
    def check_type_casting(ctx, n, dtype):
        ...
        C = te.compute((n,),
            lambda i: tvm.tir.Select(
                tvm.tir.all(*[i // block_size == tvm.tir.const(3, "int32"), i % block_size == tvm.tir.const(3, "int32")]), tvm.tir.const(1, dtype), tvm.tir.const(0, dtype)
            ),
            name="C",
        )
        s = te.create_schedule(C.op)
    ...
    check_type_casting(ctx, 16, "float32")
