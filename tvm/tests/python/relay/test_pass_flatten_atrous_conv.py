...
def compare_expected_fac(expr, expected_expr, args, tries=1):
    ...
    # tries is the count of FlattenAtrousConv pattern entries inside the expr.
    mod_flat = mod_def
    for _ in range(tries):
        mod_flat = tvm.relay.transform.FlattenAtrousConv()(mod_flat)
    ...
    result_def = (
        relay.create_executor("vm", mod=mod_def, ctx=tvm.cpu(), target="llvm")
        .evaluate()(*args)
        .asnumpy()
    )
    result_flat = (
        relay.create_executor("vm", mod=mod_flat, ctx=tvm.cpu(), target="llvm")
        .evaluate()(*args)
        .asnumpy()
    )
    result_exp = (
        relay.create_executor("vm", mod=mod_exp, ctx=tvm.cpu(), target="llvm")
        .evaluate()(*args)
        .asnumpy()
    )
