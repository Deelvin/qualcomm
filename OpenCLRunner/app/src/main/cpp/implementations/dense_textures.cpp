...
ExecTime dense_textures(JNIEnv* env, jobject assetManager) {
    std::string kernelName = "dense_textures.cl";
    ...
    size_t gws0[3] = {1024, 1, 1};
    size_t lws0[3] = {32, 1, 1};
    size_t gws1[3] = {4194304, 1, 1};
    size_t lws1[3] = {32, 1, 1};
    size_t gws2[3] = {1024, 1, 1};
    size_t lws2[3] = {32, 1, 1};
    size_t gws3[3] = {1, 4096, 1};
    size_t lws3[3] = {1, 1, 1};
    ...
    assert(err == CL_SUCCESS);
    cl_mem filter_buf = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, filter.size() * sizeof(float), filter.data(), &err);
    assert(err == CL_SUCCESS);
    ...
    assert(err == CL_SUCCESS);
    cl_kernel kernel3 = clCreateKernel(program, "fused_nn_dense_add_nn_relu_1_kernel3", &err);
    assert(err == CL_SUCCESS);
    ...
    err = clSetKernelArg(kernel1, 0, sizeof(cl_mem), (void *)&filter_img);
    assert(err == CL_SUCCESS);
    err = clSetKernelArg(kernel1, 1, sizeof(cl_mem), (void *)&filter_buf);
    assert(err == CL_SUCCESS);
    cl_event event1;
    ...
    err = clSetKernelArg(kernel2, 0, sizeof(cl_mem), (void *)&input_img);
    assert(err == CL_SUCCESS);
    err = clSetKernelArg(kernel2, 1, sizeof(cl_mem), (void *)&filter_img);
    assert(err == CL_SUCCESS);
    err = clSetKernelArg(kernel2, 2, sizeof(cl_mem), (void *)&compute);
    assert(err == CL_SUCCESS);
    cl_event event2;
    ...
    err = clSetKernelArg(kernel3, 0, sizeof(cl_mem), (void *)&output);
    assert(err == CL_SUCCESS);
    err = clSetKernelArg(kernel3, 1, sizeof(cl_mem), (void *)&compute);
    assert(err == CL_SUCCESS);
    err = clSetKernelArg(kernel3, 2, sizeof(cl_mem), (void *)&bias_buf);
    assert(err == CL_SUCCESS);
    cl_event event3;
    err = clEnqueueNDRangeKernel(command_queue, kernel3, 3, NULL, gws3, lws3, 0, NULL, &event3);
    assert(err == CL_SUCCESS);
    ...
    double kernel4TimeMS = (time_end - time_start) * 1e-6;
    __android_log_print(ANDROID_LOG_DEBUG, "OpenCL Runner Kernel size", "Kernel3 : %f ms", kernel4TimeMS);

    double kernelTimeMS = kernel1TimeMS + kernel2TimeMS + kernel3TimeMS + kernel4TimeMS;
    ...
    assert(err == CL_SUCCESS);
    err = clReleaseMemObject(filter_buf);
    assert(err == CL_SUCCESS);
