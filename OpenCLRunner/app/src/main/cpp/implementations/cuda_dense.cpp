...
ExecTime cuda_dense(JNIEnv* env, jobject assetManager) {
    std::string kernelName = "cuda_dense.cl";
    ...
    err = clSetKernelArg(kernel0, 0, sizeof(cl_mem), (void *)&input_buf);
    assert(err == CL_SUCCESS);
    err = clSetKernelArg(kernel0, 1, sizeof(cl_mem), (void *)&filter_buf);
    assert(err == CL_SUCCESS);
