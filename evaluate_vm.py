...
def downcast_fp16(func, module):
    ...
    class DowncastMutator(ExprMutator):
        ...
        def visit_call(self, call):
            ...
            return IRModule.from_expr(Call(new_fn, new_args, call.attrs))
...
class ImageNetValidator(Validator):
    ...
    def Validate(self, vm, ref_outputs=[]):
        tvm_output = vm.get_outputs()[0]
        #import ipdb; ipdb.set_trace()
...
class Deeplabv3Validator(Validator):
    ...
    def Validate(self, m, ref_outputs=[]):
        ...
        if self.dtype == "float32": 
            rtol=1e-3
            atol=1e-3
        tvm_outputs = m.get_outputs()
        for i in range(len(tvm_outputs)):
            tvm_output = tvm_outputs[i]
...
class Yolov3Validator(Validator):
    ...
    def Validate(self, m, ref_outputs=[], show=False):
        ...
        outputs = []
        for i in range(len(tvm_outputs)):
            outputs.append(tvm_outputs[i].asnumpy())
...
class Executor(object):
    ...
    def advanced_time_evaluator(self, m, func_name, ctx, number=1, repeat=1, min_repeat_ms=0, time_to_work_ms=0, cooldown_interval_ms=0, f_preproc=""):
        ...
        def ms_to_s(ms): 
            return ms / 1000
        one_run_time = m.module.time_evaluator(func_name, ctx, number=1,repeat=1,min_repeat_ms=0)("main").results[0]
        ...
        def _time_evaluator(func_name, m, ctx, number=1, repeat=1, min_repeat_ms=0, cooldown_interval_ms=0, repeats_to_cooldown=1, f_preproc=""):
            def evaluator():
                ...
                for _ in range(math.ceil(repeat / repeats_to_cooldown)):
                    time_f = m.module.time_evaluator(func_name, ctx, number=number, repeat=repeats_to_cooldown, min_repeat_ms=min_repeat_ms, f_preproc=f_preproc)("main")
        ...
        if inspect.signature(m.module.time_evaluator).parameters.get("cooldown_interval_ms"):
            time_f = m.module.time_evaluator(func_name, ctx, number=number, repeat=repeat, min_repeat_ms=min_repeat_ms, cooldown_interval_ms=cooldown_interval_ms, repeats_to_cooldown=repeats_to_cooldown, f_preproc=f_preproc)("main")
        else:
    ...
    def _benchmark(
        ...
        if self.remote:
            ...
            temp = utils.tempdir()
            dso_binary = "dev_lib_cl_vm.so"
            dso_binary_path = temp.relpath(dso_binary)
            vmc = tvm.relay.backend.vm.compile(tvm_mod, target=target, target_host=target_host, params=params)
            ...
            rexec = self.remote.load_module(dso_binary)
            # vme = tvm.runtime.vm.Executable.load_exec(code, rlib)
            if args.debug:
                vm = tvm.runtime.profiler_vm.VirtualMachineProfiler(rexec, ctx)
            else:
                vm = tvm.runtime.vm.VirtualMachine(rexec, ctx)
        else:
            ctx = tvm.device(target, 0)
            vmc = tvm.relay.backend.vm.compile(tvm_mod, target=target, target_host=target_host, params=params)
            if args.debug:
                vm = tvm.runtime.profiler_vm.VirtualMachineProfiler(vmc, ctx)    
            else:
                vm = tvm.runtime.vm.VirtualMachine(vmc, ctx)


        inputs = []
        
        if isinstance(validator, Validator):
            inputs_dict = validator.GetInputDictionary()
            for key, data in inputs_dict.items():
                inputs.append(tvm.nd.array(data.astype("float32"), ctx))
                vm.set_input("main", inputs[-1])

        elif isinstance(input_shape, dict):
            for key in input_shape:
                inputs.append(tvm.nd.array(np.random.normal(size=input_shape[key]).astype(dtype)))
                vm.set_input("main", inputs[-1])

        else:
            inputs.append(tvm.nd.array(np.random.normal(size=input_shape).astype(dtype)))
            vm.set_input("main", inputs[-1])
        
        # input_tensor = tvm.nd.array(inputs[-1], ctx)
        # vm.invoke("main", input_tensor)
        ...
        if args.debug:
            print(vm.profile())
            # time_f = self.advanced_time_evaluator(vm, "invoke", ctx, number, repeat, min_repeat_ms, time_to_work_ms, cooldown_interval_ms)
        else:
            time_f = self.advanced_time_evaluator(vm, "invoke", ctx, number, repeat, min_repeat_ms, time_to_work_ms, cooldown_interval_ms)
        ...
        if validator:
            if isinstance(validator, Validator):
                ...
                tvm_outputs = vm.get_outputs()
                for i, ref_output in enumerate(ref_outputs):
                    output = tvm_outputs[i].asnumpy()
